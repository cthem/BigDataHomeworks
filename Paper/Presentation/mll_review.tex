...
\item[$\bullet$] Relevant-nonrelevant label pairs of $x_i$: $(y_j,y_k) \in Y_i
  \times \bar Y_i$: decision hyperplane\footnote{w.r.t. singed classifier distance} is: $h_j, h_k$: $\left<w_j-w_k, x_i \right> + b_j - b_k=0$
\item[$\bullet$] Separation margin is the minimum distance of $x_i$ from every  $Y_i \times \bar Y_i$
\item[$\bullet$] SVM $\rightarrow$ minimize loss \emph{and} maximize margin.
  Approximate $max(\cdot)$ and introduce slack variables.

\item[$\bullet$] Ranks labels w.r.t. singled distance
...
\begin{itemize}
\item[$\bullet$] Encodes labels as binary random vectors $\bm{y}=\{-1,1\}\}^q$
\item[$\bullet$] For data $x,\bm{y}$ learn joint probability distribution $p(x,\bm{y})$
\item[$\bullet$] Learn pdf $p(x,\bm{y})$ via the principle of maximun entropy
  \begin{itemize}
    \item Maximize $H_p(x,y)$ subject to $K$ constraints $\mathbb{E}[f_k(x,\bm{y})] = F_k$
    \item $f_k$ models label correlations with $F_k$ learned from the training data
  \end{itemize}
\item[$\bullet$] Optimize via Lagrange multipliers, arrive at $p(\bm{y} | x)$
